...
output_file = [f for f in input_file if "_0." in f][0][:19] + "_rgb.bag"
    ...
    while k < len(input_file):
        ...
        for topic, msg, t in bag_in[k].read_messages():
            ...
            if False:
                ...
                topic = "/rgb/image_rect/compressed"
                img = bridge.imgmsg_to_cv2(msg, desired_encoding="rgb8")
                msg_compressed = bridge.cv2_to_compressed_imgmsg(img)
                bag_out.write(topic, msg_compressed, t)
                topic_bool = True
